...
test_description='reachability on a commit graph with inconsistent generation numbers

A commit with a committer date of 2^32 seconds or more (2106-02-07 or later)
used to receive a generation number that prior Git versions truncated to 32 bits
while writing the commit graph [1]. The result is a commit graph whose
generation numbers do not increase monotonically from parents to children.
...
The generation-number cutoffs behind "git merge-base --is-ancestor" and "git
tag/branch --contains" share the same weakness: they prune a commit once its
generation number drops below the smallest generation among the commits they are
looking for, so on such a commit graph they silently report a future-dated
commit as unreachable from its descendants.
...
rewrites them, so Git needs to handle them gracefully. These tests construct one
by writing a correct commit graph and then patching its GDA2 chunk, and check
that Git falls back to a full walk instead of returning a wrong answer.
...
#
# future-ancestor is an ancestor of tip, so it is their only merge base, and
# both the tip tag and the branch contain it.
...
test_expect_success PERL_TEST_HELPERS,TIME_IS_64BIT,TIME_T_IS_64BIT 'merge-base --is-ancestor is correct' '
	(
		cd repo &&
		git merge-base --is-ancestor future-ancestor tip
	)
'

test_expect_success PERL_TEST_HELPERS,TIME_IS_64BIT,TIME_T_IS_64BIT 'tag --contains finds descendant tags' '
	(
		cd repo &&
		git tag --contains future-ancestor | sort >actual &&
		git tag | sort >expect &&
		test_cmp expect actual
	)
'

test_expect_success PERL_TEST_HELPERS,TIME_IS_64BIT,TIME_T_IS_64BIT 'branch --contains finds descendant branches' '
	(
		cd repo &&
		git branch --format="%(refname:short)" --contains future-ancestor >actual &&
		git for-each-ref --format="%(refname:short)" refs/heads >expect &&
		test_cmp expect actual
	)
'
